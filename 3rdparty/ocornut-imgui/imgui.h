...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_TitleMove              = 1 << 3,   // Only move if the titlebar is hovered
    ImGuiWindowFlags_NoScrollbar            = 1 << 4,   // Disable scrollbars (window can still scroll with mouse or programatically)
    ImGuiWindowFlags_NoScrollWithMouse      = 1 << 5,   // Disable user vertically scrolling with mouse wheel
    ImGuiWindowFlags_NoCollapse             = 1 << 6,   // Disable user collapsing window by double-clicking on it
    ImGuiWindowFlags_AlwaysAutoResize       = 1 << 7,   // Resize every window to its content every frame
    ImGuiWindowFlags_ShowBorders            = 1 << 8,   // Show borders around windows and items
    ImGuiWindowFlags_NoSavedSettings        = 1 << 9,   // Never load/save settings in .ini file
    ImGuiWindowFlags_NoInputs               = 1 << 10,  // Disable catching mouse or keyboard inputs
    ImGuiWindowFlags_MenuBar                = 1 << 11,  // Has a menu-bar
    ImGuiWindowFlags_HorizontalScrollbar    = 1 << 12,  // Allow horizontal scrollbar to appear (off by default). You may use SetNextWindowContentSize(ImVec2(width,0.0f)); prior to calling Begin() to specify width. Read code in imgui_demo in the "Horizontal Scrolling" section.
    ImGuiWindowFlags_NoFocusOnAppearing     = 1 << 13,  // Disable taking focus when transitioning from hidden to visible state
    ImGuiWindowFlags_NoBringToFrontOnFocus  = 1 << 14,  // Disable bringing window to front when taking focus (e.g. clicking on it or programatically giving it focus)
    ImGuiWindowFlags_AlwaysVerticalScrollbar= 1 << 15,  // Always show vertical scrollbar (even if ContentSize.y < Size.y)
    ImGuiWindowFlags_AlwaysHorizontalScrollbar=1<< 16,  // Always show horizontal scrollbar (even if ContentSize.x < Size.x)
